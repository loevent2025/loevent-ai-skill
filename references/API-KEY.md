...
本文件覆盖两条路:
- **默认走 Gemini**:只需 `GEMINI_API_KEY` —— 见规则一~三;
- **用 Gemini 以外的模型**(任意 OpenAI 兼容供应商,如 GLM/DeepSeek/豆包/OpenAI):填一组 `LOEVENT_TEXT_*` 等环境变量 —— 见规则四。
...
- **Gemini** → 走规则一~三收 `GEMINI_API_KEY`;
- **其它模型(GLM / DeepSeek / Kimi / 豆包 / OpenAI…)** → 走规则四收 `LOEVENT_TEXT_*`;
...
用户选了其它模型(规则零),或脚本报 `缺 LOEVENT_TEXT_API_KEY`/`缺 model` 时,收集**一组**字段写进同一个项目根 `.env`(写法同规则三)。任意 OpenAI 兼容供应商,换 base_url 即可;**官方实测背书:Gemini、GLM;其余为「理论兼容,自行验证」**。
...
**② 联网调研(trends/guests/event-strategy/budget 才需要)**:**混合策略——有原生搜索的家默认走原生、没原生的走外部**:
- **有原生搜索的家**(Qwen / GLM / ERNIE)→ 默认直接用它的原生联网搜索,**不用额外配**;
- **没原生的家**(DeepSeek、Kimi、豆包等)→ 配外部:`LOEVENT_SEARCH_PROVIDER=tavily` 或 `bocha`(都有免费额度)+ `LOEVENT_SEARCH_API_KEY`;
- **想强制统一走外部**(质量可控、不依赖各家原生)→ 显式配 `LOEVENT_SEARCH_PROVIDER`,会**覆盖**原生;
- **既没原生、也没配外部** → 默认**报错中止**(避免无来源编造);接受无实时来源设 `LOEVENT_ALLOW_UNGROUNDED=1`。
...
- 海报「文字可编辑」的**消字**(可选):`LOEVENT_IMAGE_EDIT_PROVIDER=qwen` + `LOEVENT_IMAGE_EDIT_MODEL=qwen-image-edit` + `LOEVENT_IMAGE_EDIT_API_KEY`(DashScope);不配则用 Gemini 消字。**注:抹字必须有图像 API(Gemini 或 qwen),没有本地抹字兜底——两者都无则「文字可编辑」的抹字这步做不了。**
- 海报「文字可编辑」的**文字定位**(可选,Google OCR 用不了时):`LOEVENT_OCR_PROVIDER=qwen-vl`(或 `glm-4v`)+ `LOEVENT_OCR_MODEL` + `LOEVENT_OCR_API_KEY` —— 用多模态 VL 估文字框;不配则有 GCV 走 GCV 精确、再没有就看图估位(VL 框精度不及 GCV,编辑器里拖准)。

**一组好上手的组合(示例)**:`LOEVENT_TEXT_PROVIDER=glm` + `LOEVENT_SEARCH_PROVIDER=tavily` + `LOEVENT_IMAGE_PROVIDER=openai`(或 `doubao`/`cogview`)——各家都有免费档,按你能拿到的 key 自由搭配。

...
**② 联网调研(trends/guests/company/budget 才需要)**:**混合策略——有原生搜索的家默认走原生、没原生的走外部**:
- **有原生搜索的家**(Qwen / GLM / ERNIE)→ 默认直接用它的原生联网搜索,**不用额外配**;
- **没原生的家**(DeepSeek、Kimi、豆包等)→ 配外部:`LOEVENT_SEARCH_PROVIDER=bocha`(国内,免费 1000 次)或 `tavily`(海外)+ `LOEVENT_SEARCH_API_KEY`;

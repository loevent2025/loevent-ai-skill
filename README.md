...
| "做张活动海报" / "把海报上的字改一下" | 按选定风格生成海报；还可消字后让文字可编辑重排（进阶，需 Vision 服务账号） |
...
4. **（可选·进阶）海报"文字可编辑"** —— 若想让海报上的文字可改（消字后用系统字体重排），还需一把 **Google Cloud Vision 服务账号**（`GOOGLE_APPLICATION_CREDENTIALS` 指向其 JSON，独立于 Gemini Key）。**普通出图不需要它**。

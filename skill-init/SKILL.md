...
- **同一个活动内、用户提新的子需求**(如已做受众、现在要海报):**复用**沙箱里的 `event/host/plan.json`,**只重新生成被点名的那个产物**(海报/文案/预算每次跑本就覆盖重生);**不要**把 event-strategy(20+ 次联网)/trends/guests 这种重调研无谓重跑——又慢又烧 Key。
...
>
> 已生成本地档案(event/host/plan.json)。**接下来怎么走:**
> - 想要**完整策划方案** → 走主线:**受众画像 → 主办方调研(出 3 套策略方向)→ 你选一套 → 完整方案**;
> - 只想要**单项** → 直接说:海报 / 预算 / 时间线 / 社媒文案 / Luma 文案 / 嘉宾简介 …
>
> 要哪个?
...
- 结尾给**主线路标,不是平铺清单**:要完整方案就按"受众 → 调研 → 选方向 → 方案"主线走,只想单项可直接点;别让用户面对一堆并列选项却不知从哪开始。
